...
const fallbackOverview = {
  portfolioValue: 24860.42, portfolioChange: 7.84, cookBalance: 8420.55, stakedValue: 12800,
  tvl: 48200000, activeWallets: 18420, cookPrice: 0.0842, volume24h: 1240000, faucetCooldown: 0,
};
const fallbackMarkets = [
  { id: 'cook-usdc', pair: 'COOK / USDC', tokenA: 'COOK', tokenB: 'USDC', price: .0842, change24h: 8.42, volume24h: 426000, tvl: 12800000, feeTier: .3, sparkline: [28, 31, 29, 35, 32, 40, 39, 48, 46, 55, 52, 62] },
  { id: 'cook-eth', pair: 'COOK / ETH', tokenA: 'COOK', tokenB: 'ETH', price: .000025, change24h: -1.28, volume24h: 184000, tvl: 6200000, feeTier: .3, sparkline: [52, 49, 54, 51, 47, 48, 43, 45, 42, 44, 41, 40] },
  { id: 'usdc-eth', pair: 'USDC / ETH', tokenA: 'USDC', tokenB: 'ETH', price: 2568.2, change24h: 2.18, volume24h: 882000, tvl: 19400000, feeTier: .05, sparkline: [34, 36, 35, 40, 42, 39, 45, 46, 49, 48, 51, 54] },
];
const fallbackActivity = [
  { id: 'a1', type: 'swap', label: 'Swapped COOK for USDC', amount: 1280, token: 'COOK', status: 'success', timestamp: '12 min ago', hash: '0x8f2a…41c9' },
  { id: 'a2', type: 'stake', label: 'Staked into Golden Crumb', amount: 4200, token: 'COOK', status: 'success', timestamp: '2 hr ago', hash: '0x2b19…c882' },
  { id: 'a3', type: 'claim', label: 'Claimed faucet tokens', amount: 2500, token: 'USDC', status: 'success', timestamp: 'Yesterday', hash: '0x6c40…de11' },
  { id: 'a4', type: 'provide', label: 'Added liquidity to COOK / USDC', amount: 840, token: 'USDC', status: 'pending', timestamp: 'Yesterday', hash: '0x71aa…0f29' },
];
const fallbackVaults = [
  { id: 'v1', name: 'Golden Crumb', token: 'COOK', apy: 18.42, tvl: 12800000, lockup: 'Flexible', userStaked: 4200, color: '#f3b34b' },
  { id: 'v2', name: 'Butterfly USDC', token: 'USDC', apy: 11.8, tvl: 8400000, lockup: '14 days', userStaked: 0, color: '#6bd4b0' },
...
function Logo() {
  return <div className="flex items-center gap-3">
    <div className="relative grid size-9 place-items-center rounded-xl bg-primary text-primary-foreground shadow-[0_8px_24px_rgba(243,179,75,.2)]">
      <span className="text-lg font-extrabold tracking-[-.08em]">C</span>
      <span className="absolute -right-1 -top-1 size-2 rounded-full bg-emerald-300" />
    </div>
    <div><div className="text-[13px] font-extrabold tracking-[.14em] text-foreground">COOKIE<span className="text-primary">NEXUS</span></div><div className="font-mono text-[9px] tracking-[.16em] text-muted-foreground">PROTOCOL CONSOLE</div></div>
  </div>;
}
...
function PageHeading({ eyebrow, title, detail, action }: { eyebrow: string; title: string; detail: string; action?: ReactNode }) {
  return <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-2 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.2em] text-primary"><span className="h-px w-5 bg-primary" />{eyebrow}</div><h1 className="text-2xl font-extrabold tracking-[-.045em] text-foreground sm:text-3xl">{title}</h1><p className="mt-2 max-w-2xl text-[12px] leading-5 text-muted-foreground">{detail}</p></div>{action}</div>;
}

function StatCard({ label, value, change, icon: Icon, accent = 'amber', detail }: { label: string; value: string; change?: string; icon: typeof Wallet; accent?: 'amber' | 'green' | 'blue'; detail?: string }) {
  const tones = { amber: 'text-primary bg-primary/10', green: 'text-emerald-300 bg-emerald-300/10', blue: 'text-sky-300 bg-sky-300/10' };
  return <div className="glass group rounded-2xl p-4 transition-transform duration-300 hover:-translate-y-0.5 sm:p-5"><div className="flex items-start justify-between"><div className={`grid size-8 place-items-center rounded-lg ${tones[accent]}`}><Icon size={16} /></div>{change && <span className={`font-mono text-[10px] ${change.startsWith('-') ? 'text-red-300' : 'text-emerald-300'}`}>{change}</span>}</div><div className="mt-5 text-[10px] uppercase tracking-[.12em] text-muted-foreground">{label}</div><div className="mt-1 text-xl font-extrabold tracking-[-.04em] sm:text-2xl">{value}</div>{detail && <div className="mt-1 text-[10px] text-muted-foreground">{detail}</div>}</div>;
}

function Sparkline({ points, negative = false }: { points: number[]; negative?: boolean }) {
  const max = Math.max(...points); const min = Math.min(...points); const d = points.map((point, i) => `${(i / (points.length - 1)) * 100},${36 - ((point - min) / (max - min || 1)) * 29}`).join(' ');
  return <svg viewBox="0 0 100 40" preserveAspectRatio="none" className={`h-10 w-24 ${negative ? 'text-red-300' : 'text-emerald-300'}`}><polyline fill="none" stroke="currentColor" strokeWidth="2" points={d} vectorEffect="non-scaling-stroke" /><polyline fill="none" stroke="currentColor" strokeWidth="8" opacity=".06" points={d} vectorEffect="non-scaling-stroke" /></svg>;
}

function LoadingBlocks({ count = 3 }: { count?: number }) { return <div className="grid gap-4 sm:grid-cols-3">{Array.from({ length: count }).map((_, i) => <div key={i} className="h-28 animate-pulse rounded-2xl border border-white/[.06] bg-white/[.035]" />)}</div>; }
...
function Overview() {
  ...
  return <div className="animate-rise">
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-[.9fr_1.1fr]">
      <section className="relative overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/[.17] via-primary/[.06] to-transparent p-5"><div className="pointer-events-none absolute -right-8 -top-16 size-52 rounded-full border border-primary/15 opacity-60" /><div className="pointer-events-none absolute -right-1 -top-8 size-36 rounded-full border border-primary/10" /><div className="relative"><div className="flex items-center gap-2 text-primary"><Droplets size={16} /><span className="font-mono text-[10px] uppercase tracking-[.18em]">Testnet faucet</span></div><h2 className="mt-4 text-lg font-extrabold tracking-[-.03em]">Top up your operator wallet.</h2><p className="mt-2 max-w-sm text-[11px] leading-5 text-muted-foreground">Claim a small amount of COOK or USDC to prototype your next move on Cookie Chain.</p><div className="mt-5 flex flex-col gap-2 sm:flex-row"><input value={wallet} onChange={(event) => setWallet(event.target.value)} placeholder="Wallet address" className="min-w-0 flex-1 rounded-xl border border-white/[.1] bg-black/20 px-3 py-2.5 font-mono text-[11px] outline-none placeholder:text-muted-foreground/70 focus:border-primary/50" data-testid="input-faucet-wallet" /><select value={asset} onChange={(event) => setAsset(event.target.value as 'COOK' | 'USDC')} className="rounded-xl border border-white/[.1] bg-[#191b27] px-3 py-2.5 text-[11px] font-bold outline-none" data-testid="select-faucet-asset"><option value="COOK">COOK</option><option value="USDC">USDC</option></select><button onClick={claim} disabled={faucet.isPending} className="rounded-xl bg-primary px-4 py-2.5 text-[11px] font-extrabold text-primary-foreground transition-transform hover:-translate-y-0.5 disabled:opacity-60" data-testid="button-claim-faucet">{faucet.isPending ? 'Claiming…' : 'Claim tokens'}</button></div>{notice && <div className={`mt-3 text-[10px] ${notice.tone === 'error' ? 'text-red-300' : 'text-emerald-300'}`} data-testid="status-faucet">{notice.text}</div>}</div></section>
      <section className="glass flex flex-col justify-between rounded-2xl p-5"><div><div className="flex items-center gap-2 text-muted-foreground"><Gauge size={16} /><span className="font-mono text-[10px] uppercase tracking-[.18em]">Nexus health</span></div><div className="mt-4 flex items-end justify-between"><div><div className="text-3xl font-extrabold tracking-[-.06em] text-emerald-300">99.98%</div><div className="mt-1 text-[10px] text-muted-foreground">Network availability · last 30 days</div></div><div className="rounded-lg border border-emerald-300/20 bg-emerald-300/10 px-2 py-1.5 font-mono text-[9px] text-emerald-300">ALL SYSTEMS GO</div></div></div><div className="mt-6 flex items-end gap-1"><div className="h-8 flex-1 rounded-sm bg-emerald-300/20" /><div className="h-6 flex-1 rounded-sm bg-emerald-300/20" /><div className="h-10 flex-1 rounded-sm bg-emerald-300/30" /><div className="h-7 flex-1 rounded-sm bg-emerald-300/20" /><div className="h-12 flex-1 rounded-sm bg-emerald-300/40" /><div className="h-9 flex-1 rounded-sm bg-emerald-300/30" /><div className="h-11 flex-1 rounded-sm bg-emerald-300/50" /><div className="h-10 flex-1 rounded-sm bg-emerald-300/40" /><div className="h-12 flex-1 rounded-sm bg-emerald-300/60" /><div className="h-12 flex-1 rounded-sm bg-emerald-300/50" /><div className="h-14 flex-1 rounded-sm bg-emerald-300/70" /></div></section>
    </div>
  </div>;
}
...
function LaunchpadPage() {
  const launch = useCreateNexusLaunch(); const [form, setForm] = useState({ name: '', symbol: '', supply: '1000000', creator: '' }); const [created, setCreated] = useState<any>(null); const [notice, setNotice] = useState<Notice>(null);
  const update = (key: keyof typeof form, value: string) => setForm({ ...form, [key]: value });
  const submit = () => { if (form.name.length < 2 || form.symbol.length < 2 || form.creator.length < 8 || Number(form.supply) < 1) { setNotice({ tone: 'error', text: 'Complete all fields with valid launch parameters.' }); return; } launch.mutate({ data: { name: form.name, symbol: form.symbol.toUpperCase(), supply: Number(form.supply), creator: form.creator } }, { onSuccess: (data) => { setCreated(data); setNotice({ tone: 'success', text: 'Launch draft created and queued for review.' }); }, onError: () => setNotice({ tone: 'error', text: 'Could not create draft. Check your creator wallet.' }) }); };
  return <div className="animate-rise"><PageHeading eyebrow="Creator console / 04" title="Launch the next primitive." detail="Draft a token launch with the same operator-grade controls used across the Cookie Chain ecosystem." action={<div className="flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/[.07] px-3 py-2 font-mono text-[10px] text-primary"><Rocket size={14} />Launchpad beta</div>} /><div className="grid gap-4 xl:grid-cols-[1fr_360px]"><section className="glass rounded-2xl p-5 sm:p-7"><div className="mb-7 flex items-center gap-3"><div className="grid size-9 place-items-center rounded-xl bg-primary/10 text-primary"><Sparkles size={17} /></div><div><div className="text-sm font-bold">New launch draft</div><div className="mt-1 text-[10px] text-muted-foreground">Step 1 of 1 · basic token configuration</div></div></div><div className="grid gap-5 sm:grid-cols-2"><label className="space-y-2"><span className="font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Token name</span><input value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="e.g. Crumb Club" className="w-full rounded-xl border border-white/[.09] bg-black/20 px-3.5 py-3 text-xs outline-none placeholder:text-muted-foreground/60 focus:border-primary/45" data-testid="input-launch-name" /></label><label className="space-y-2"><span className="font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Ticker symbol</span><input value={form.symbol} onChange={(e) => update('symbol', e.target.value)} placeholder="e.g. CRUMB" maxLength={8} className="w-full rounded-xl border border-white/[.09] bg-black/20 px-3.5 py-3 text-xs uppercase outline-none placeholder:text-muted-foreground/60 focus:border-primary/45" data-testid="input-launch-symbol" /></label><label className="space-y-2"><span className="font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Total supply</span><input type="number" value={form.supply} onChange={(e) => update('supply', e.target.value)} className="w-full rounded-xl border border-white/[.09] bg-black/20 px-3.5 py-3 text-xs outline-none focus:border-primary/45" data-testid="input-launch-supply" /></label><label className="space-y-2"><span className="font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Creator wallet</span><input value={form.creator} onChange={(e) => update('creator', e.target.value)} placeholder="0x…" className="w-full rounded-xl border border-white/[.09] bg-black/20 px-3.5 py-3 font-mono text-xs outline-none placeholder:text-muted-foreground/60 focus:border-primary/45" data-testid="input-launch-creator" /></label></div><div className="mt-6 rounded-xl border border-primary/15 bg-primary/[.05] p-4"><div className="flex gap-3"><ShieldCheck className="mt-0.5 shrink-0 text-primary" size={16} /><div><div className="text-[11px] font-bold">Creator safety check</div><p className="mt-1 text-[10px] leading-5 text-muted-foreground">Drafts are non-custodial. Creating a draft does not deploy a token or move funds. You can review metadata before any future deployment.</p></div></div></div><button onClick={submit} disabled={launch.isPending} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3.5 text-xs font-extrabold text-primary-foreground hover:shadow-[0_12px_30px_rgba(243,179,75,.16)] disabled:opacity-60" data-testid="button-create-launch">{launch.isPending ? 'Creating draft…' : 'Create launch draft'}<ArrowUpRight size={15} /></button>{notice && <div className={`mt-3 text-center text-[10px] ${notice.tone === 'error' ? 'text-red-300' : 'text-emerald-300'}`} data-testid="status-launch">{notice.text}</div>}</section><aside className="space-y-4"><section className="glass rounded-2xl p-5"><div className="font-mono text-[9px] uppercase tracking-[.16em] text-muted-foreground">Live preview</div><div className="mt-5 rounded-2xl border border-white/[.07] bg-gradient-to-br from-[#262233] to-[#11131d] p-5"><div className="flex items-center gap-3"><div className="grid size-12 place-items-center rounded-2xl bg-primary text-lg font-extrabold text-primary-foreground">{form.symbol ? form.symbol[0].toUpperCase() : '?'}</div><div><div className="text-sm font-bold">{form.name || 'Your token name'}</div><div className="mt-1 font-mono text-[10px] text-primary">{form.symbol ? `$${form.symbol.toUpperCase()}` : '$TICKER'}</div></div></div><div className="mt-7 grid grid-cols-2 gap-3 border-t border-white/[.08] pt-4"><div><div className="font-mono text-[9px] text-muted-foreground">SUPPLY</div><div className="mt-1 text-xs font-bold">{Number(form.supply || 0).toLocaleString()}</div></div><div><div className="font-mono text-[9px] text-muted-foreground">STATUS</div><div className="mt-1 text-xs font-bold text-primary">{created?.status || 'DRAFT'}</div></div></div></div></section><section className="glass rounded-2xl p-5"><div className="flex items-center gap-2 text-emerald-300"><Blocks size={15} /><span className="text-xs font-bold">Launchpad activity</span></div><div className="mt-4 space-y-3 text-[10px] text-muted-foreground"><div className="flex justify-between"><span>Draft creation</span><span className="text-emerald-300">Ready</span></div><div className="flex justify-between"><span>Metadata review</span><span>Next</span></div><div className="flex justify-between"><span>Deployment</span><span>Optional</span></div></div></section></aside></div></div>;
}
...
function CanvasPage() {
  const [cells, setCells] = useState<Record<string, string>>({}); const [color, setColor] = useState('#f3b34b'); const colors = ['#f3b34b', '#e88459', '#6bd4b0', '#8aa0ff', '#cf83db', '#e7e1cb'];
  const paint = (key: string) => setCells({ ...cells, [key]: cells[key] === color ? '' : color });
  return <div className="animate-rise"><PageHeading eyebrow="Community / 06" title="Leave your mark." detail="A shared pixel surface for Cookie Chain operators. Paint a signal, spell a name, claim a tiny square of the network." action={<div className="flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/[.07] px-3 py-2 font-mono text-[10px] text-primary"><Grid3X3 size={14} />{Object.values(cells).filter(Boolean).length} pixels placed</div>} /><div className="grid gap-4 xl:grid-cols-[1fr_300px]"><section className="glass rounded-2xl p-4 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><div className="text-sm font-bold">The Nexus wall</div><div className="mt-1 text-[10px] text-muted-foreground">Click any square to paint · your changes are local to this session</div></div><button onClick={() => setCells({})} className="rounded-lg border border-white/[.08] px-3 py-2 text-[10px] font-bold text-muted-foreground hover:border-red-300/30 hover:text-red-300" data-testid="button-clear-canvas">Clear paint</button></div><div className="mx-auto grid max-w-[720px] grid-cols-20 gap-1 rounded-xl border border-white/[.08] bg-[#0a0c12] p-2 sm:gap-1.5 sm:p-3">{Array.from({ length: 240 }).map((_, index) => { const key = String(index); return <button key={key} onClick={() => paint(key)} className="aspect-square rounded-[2px] bg-white/[.045] transition-transform hover:scale-125 hover:bg-white/[.14]" style={cells[key] ? { backgroundColor: cells[key], boxShadow: `0 0 10px ${cells[key]}77` } : undefined} aria-label={`Paint canvas pixel ${index + 1}`} data-testid={`button-pixel-${index}`} />; })}</div></section><aside className="space-y-4"><section className="glass rounded-2xl p-5"><div className="font-mono text-[9px] uppercase tracking-[.16em] text-muted-foreground">Palette</div><div className="mt-4 grid grid-cols-3 gap-2">{colors.map((item) => <button key={item} onClick={() => setColor(item)} className={`grid aspect-square place-items-center rounded-xl border transition-transform hover:scale-105 ${color === item ? 'border-primary ring-2 ring-primary/20' : 'border-white/[.08]'}`} style={{ backgroundColor: `${item}22` }} data-testid={`button-palette-${item.slice(1)}`}><span className="size-4 rounded-full" style={{ backgroundColor: item }} /></button>)}</div><div className="mt-4 flex items-center gap-2 rounded-lg bg-white/[.04] p-2 font-mono text-[10px] text-muted-foreground"><span className="size-3 rounded-full" style={{ backgroundColor: color }} />Selected ink <span className="ml-auto text-foreground">{color}</span></div></section><section className="glass rounded-2xl p-5"><div className="flex items-center gap-2"><Target size={15} className="text-primary" /><span className="text-xs font-bold">Canvas protocol</span></div><p className="mt-3 text-[10px] leading-5 text-muted-foreground">The wall is a low-stakes social layer for the chain. Paint is ephemeral in this demo; a future release will anchor murals on-chain.</p></section></aside></div></div>;
}
